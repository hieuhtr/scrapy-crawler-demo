...
class TikiSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        try:
            i['current_price'] = response.xpath('//p[@class="special-price-item"]/@data-value').extract()[0].strip().encode('UTF-8')
        except Exception:
            pass

        try:
            i['original_price'] = response.xpath('//p[@class="old-price-item"]/@data-value').extract()[0].strip().encode('UTF-8')
        except Exception:
